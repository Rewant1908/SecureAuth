...
class SecurityProtection:
    ...
    def check_brute_force(self, username: str, ip_address: str) -> Tuple[bool, Optional[str]]:
        ...
        if failed_count >= self.max_login_attempts:
            # Account is locked - get the unlock time
            cursor.execute("""
    ...
    def record_login_attempt(self, user_id: Optional[int], username: str, 
    ...
    def check_rate_limit(self, identifier: str) -> Tuple[bool, int]:
        ...
        # Clean old entries from this identifier
    ...
    def detect_credential_stuffing(self, ip_address: str, time_window_minutes: int = 5) -> bool:
        ...
        Credential stuffing = trying many different usernames from same IP
        (using stolen username/password lists)
        ...
        # If trying many different usernames in short time = credential stuffing
        return unique_usernames > 10
    
    def log_security_event(self, user_id: Optional[int], event_type: str, 
                          severity: str, details: str, ip_address: str = None):
        """
...
if __name__ == "__main__":
    ...
    for i in range(3):
        security.record_login_attempt(
            None, "testuser", "192.168.1.100", 
            "Chrome", False, "Invalid password"
        )
    ...
    # Test 2: Trigger brute force lockout
    print("-" * 60)
    ...
    is_locked, unlock_time = security.check_brute_force("testuser", "192.168.1.100")
    print(f"  After 6 total failed attempts:")
    ...
    print("-" * 60)
    for i in range(15):
        allowed, remaining = security.check_rate_limit("192.168.1.100")
        if i < 5 or i == 14:
            status = "✓ Allowed" if allowed else "✗ BLOCKED"
            print(f"  Request {i+1}: {status} (Remaining: {remaining})")
    print()
    
    # Test 4: Credential stuffing detection
    print("-" * 60)
    ...
    for username in usernames:
        security.record_login_attempt(
            None, username, "10.0.0.5",
            "Chrome", False, "Invalid credentials"
        )
    ...
    security.log_security_event(
        1, "brute_force_attempt", "high",
        "Multiple failed login attempts", "192.168.1.100"
    )
    security.log_security_event(
        2, "suspicious_activity", "medium",
        "Login from new location", "203.0.113.42"
    )
    ...
    for agent, expected in test_agents:
        is_sus = security.is_suspicious_user_agent(agent)
        status = "✓ Correct" if is_sus == expected else "✗ Wrong"
        print(f"  {status} - '{agent[:40]}...' - Suspicious: {is_sus}")
    print()
